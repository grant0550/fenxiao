...
define('CLOUD_GATEWAY_URL', 'http://localhost');
define('CLOUD_GATEWAY_URL_NORMAL', 'http://localhost');
...
function cloud_prepare() {
    ...
    // if (empty($_W['setting']['site']['key']) || empty($_W['setting']['site']['token'])) {
        // return error('-1', "您的站点只有在黑锐资源云服务平台成功注册后，才能使用云更新功能。");
    // }
    return true;
}
function cloud_build() {
    ...
    $dat = cloud_request('http://localhost', $pars);
...
function cloud_download($path, $type = '') {
    ...
    $dat = cloud_request('http://localhost', $pars, $headers, 300);
    if (is_error($dat)) {
...
function cloud_sms_send($mobile, $content, $postdata = array()) {
    ...
    $response = cloud_request('http://localhost', $pars);
...
function cloud_cron_create($cron) {
    ...
    $result = cloud_request('http://localhost', $pars);
...
function cloud_auth_url($forward, $data = array()) {
    ...
    $auth_url = 'http://we7cloud.tubaoge.com/api/auth.php?__auth=' . $query;
    return $auth_url;
}
...
function cloud_flow_master_post($flow_master) {
    ...
    $dat = cloud_request('http://localhost', $pars, array() , 300);
    if (is_error($dat)) {
        return error(-1, '网络存在错误， 请稍后重试。' . $dat['message']);
    }
