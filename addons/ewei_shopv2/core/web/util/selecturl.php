...
class Selecturl_EweiShopV2Page extends WebPage
{
	...
	public function check_perm($str)
	{
		...
		$permsInfo = pdo_fetch('SELECT perms2 FROM ' . tablename('ewei_shop_perm_user') . ' WHERE uid=:uid  AND username=:username ', array(':uid' => $_W['user']['uid']));
		...
		$allUrls = array(
			array(
				...
				'list' => array(
					...
					array('name' => '全返记录', 'url' => mobileUrl('member/fullback/get_list', NULL, $full), 'url_wxapp' => '/pages/commission/return/index'),
					array('name' => '计次时商品', 'url' => mobileUrl('verifygoods/get_list', NULL, $full), 'url_wxapp' => '/pages/verifygoods/index'),
					array('name' => '积分商城', 'url' => mobileUrl(NULL, NULL, $full), 'url_wxapp' => '/pages/creditshop/index'),
					array('name' => '分销中心', 'url' => mobileUrl(NULL, NULL, $full), 'url_wxapp' => '/pages/commission/register/index'),
                    array('name' => '商户街[单独收费]', 'url' => '', 'url_wxapp' => '/pages/changce/merch/index')
					)
				),
			array(
				'name' => '商品属性',
